...
KademliaNode::KademliaNode(const std::string &id, const std::string &ip, int port, int dht_port)
    : node_id(id), ip(ip), port(port), dht_port(dht_port) {
...
std::string KademliaNode::find_value(const std::string &key) {
    ...
    if (storage.find(key) != storage.end()) {
        ...
        for (const auto &peer : peers) {
            std::cout << "Querying peer " << peer.first << ":" << peer.second << " for key: " << key << std::endl;
            std::string value = send_find_request(peer.first, peer.second, key);
            if (!value.empty()) {
                std::cout << "Received value from peer " << peer.first << ":" << peer.second << " for key: " << key << std::endl;
                return value;
            }
        }
    }
...
void KademliaNode::add_peer(const std::string &peer_ip, int peer_port) {
    // 자신을 피어 목록에 추가하지 않음
    if (peer_ip == ip && peer_port == dht_port) {
        return;
    }

    std::cout << "Adding peer: " << peer_ip << ":" << peer_port << std::endl;
...
void KademliaNode::start_server() {
    std::thread([this]() {
        ...
        address.sin_port = htons(dht_port);
...
void KademliaNode::join_network(const std::string &bootstrap_ip, int bootstrap_port) {
    std::string message = "JOIN " + ip + " " + std::to_string(dht_port);
    ...
    // 피어 탐색 시작
    for (const auto &peer : peers) {
        std::cout << "Sending peer request to: " << peer.first << ":" << peer.second << std::endl;
        std::string peer_response = send_peer_request(peer.first, peer.second);
        std::cout << "Received peer response from " << peer.first << ":" << peer.second << " - " << peer_response << std::endl;
        std::istringstream peer_iss(peer_response);
        while (peer_iss >> peer_ip >> peer_port) {
            std::cout << "Received peer from peer request: " << peer_ip << ":" << peer_port << std::endl;
            add_peer(peer_ip, peer_port);
        }
    }
...
void KademliaNode::handle_client(int socket) {
    ...
    if (request.substr(0, 8) == "FIND_KEY") {
	    ...
	    std::cout << "Handling FIND_KEY request for key: " << key << std::endl;
	    std::string value = find_value(key);
	    if (value.empty()) {
		    value = "Client ID not found";
	    }
	    send(socket, value.c_str(), value.size(), 0);
	    std::cout << "Sent response: " << value << std::endl;
    ...
    }else if (request.substr(0, 15) == "REQUEST_PEER_LIST") {
	    // 요청한 노드에게 피어 목록을 보냄
	    std::ostringstream oss;
	    {
		    std::lock_guard<std::mutex> lock(mtx);
		    for (const auto &peer : peers) {
			    oss << peer.first << " " << peer.second << " ";
		    }
	    }
	    std::string peer_list = oss.str();
	    send(socket, peer_list.c_str(), peer_list.size(), 0);
    }
...
std::string KademliaNode::send_find_request(const std::string &peer_ip, int peer_port, const std::string &key) {
    std::cout << "Sending FIND_KEY request for key: " << key << " to " << peer_ip << ":" << peer_port << std::endl;
    return send_message(peer_ip, peer_port, "FIND_KEY " + key);
}

std::string KademliaNode::send_peer_request(const std::string &peer_ip, int peer_port) {
    return send_message(peer_ip, peer_port, "REQUEST_PEER_LIST");
}
